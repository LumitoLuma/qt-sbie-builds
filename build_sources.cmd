if %1 == x64 (
    ...
    perl Configure shared --prefix=%~dp0openssl VC-WIN64A
    jom
    jom install
    popd
    pushd %~dp0src\
    call configure.bat -release -opensource -confirm-license -prefix %~dp0bin\5.15.13\msvc2019_64 -platform win32-msvc -openssl-linked OPENSSL_PREFIX="%~dp0openssl" -nomake tests -nomake examples -skip qtdoc -skip qtwebengine
    jom
...
pushd %~dp0src\openssl\
    ...
    popd
    pushd %~dp0src\
    call configure.bat -release -opensource -confirm-license -prefix %~dp0bin\5.15.13\msvc2019 -platform win32-msvc -openssl-linked OPENSSL_PREFIX="%~dp0openssl" -nomake tests -nomake examples -skip qtdoc -skip qtwebengine
    jom
    if %ERRORLEVEL% == 0 jom install
